...
impl DerObject {
    /// Creates a new DER object from `tag` and `value` (the length is computed
    /// from `value`)
    ...
    /// DER-serializes `self` into `buf` and returns the amount of bytes
    /// written
    pub fn serialize<'a>(
        &self,
    ...
    /// Efficiently computes the length of the DER-serialized representation of
    /// an object with `payload_len` payload bytes
