...
macro_rules! impl_conv {
    ...
    (impl: $uint:ident) => (
        fn $uint(self) -> Result<$uint, Asn1DerError> {
            if self > $uint::max_value() as u128 {
                Err(Asn1DerError::Unsupported)
            } else {
                Ok(self as $uint)
            }
        }
...
impl FromDerObject for u128 {
    fn from_der_object(der_object: DerObject) -> Result<Self, Asn1DerError> {
        ...
        if der_object.tag != DerTag::Integer {
            return Err(Asn1DerError::InvalidTag);
        }
        if der_object.value.data.is_empty() {
        ...
        if unsigned {
            match data.peek().and_then(|b| Some(**b)) {
        ...
        if data.len() > 16 {
            return Err(Asn1DerError::Unsupported);
        }
